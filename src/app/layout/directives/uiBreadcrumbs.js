(function (module) {
  ...
  module.directive('uiBreadcrumbs', ['$interpolate', '$state', function ($interpolate, $state) {
    return {
      restrict: 'E',
      templateUrl: 'app/layout/directives/uiBreadcrumbs.tpl.html',
      scope: {
        displaynameProperty: '@',
        abstractProxyProperty: '@?',
        abstractDisplayProperty: '@?'
      },
      link: function (scope) {
        scope.breadcrumbs = [];
          ...
          while (currentState && currentState.name !== '') {
            ...
            if (workingState) {
              ...
              if (displayName !== false && !stateAlreadyInBreadcrumbs(workingState, breadcrumbs)) {
				// If workingState.abstract, set the route undefined as it is only for display purposes in breadcrumbs
                breadcrumbs.push({
                  displayName: displayName,
                  route: workingState.abstract ? undefined : workingState.name
                });
              }
            }
		 ...
		 * set it to `false` which means this breadcrumb level will be skipped entirely. If the scope.abstractDisplayProperty
		 * is defined and true, then we return the currentState regardless being an abstract state
		 * @param currentState
		 * @returns {*}
		 */
        function getWorkingState(currentState) {
          ...
          if (currentState.abstract === true) {
            if (typeof scope.abstractDisplayProperty !== 'undefined') {
              display = getObjectValue(scope.abstractDisplayProperty, currentState);
            }
            if (!display && typeof scope.abstractProxyProperty !== 'undefined') {
			  ...
			  // If display, then we return the currentState to be added to the breadcrumb array
              workingState = display ? currentState : false;
            }
          }
          return workingState;
        }
